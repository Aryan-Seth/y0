...
from utils import ZIdentification,Identification, Unidentifiable
...
def identify(identification: ZIdentification) -> Expression:
    ...
    I,J represent currently active interventions, they function as
    """
    ...
    if(len(Z_w.union(W))==0):
        Z_updated=Z-Z_w
        return identify(line_3(identification,W,Z_w,Z_updated))

    # line 4
    graph_without_treatments = graph.remove_nodes_from(treatments.union(I).union(J))
    if not graph_without_treatments.is_connected():
        expression = Product.safe(map(identify, line_4(identification,Z)))
        return Sum.safe(
            expression=expression,
            ranges=vertices.difference(outcomes | treatments.union(I)),
        )

    #pre-condition for debugging -line 5
    if(graph_without_treatments.is_connected()):
    # line 5    
        ...
        if district_without_treatment in graph.districts():
            ...
            expression = Product.safe(p_parents(v, parents-(I.union(J))) for v in district_without_treatment)
...
def line_3(identification: ZIdentification,w,z_w,Z_temp) -> ZIdentification:
    ...
    # no_effect_on_outcome = graph.get_no_effect_on_outcomes(treatments.union(w), outcomes)
